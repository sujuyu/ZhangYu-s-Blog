...
最近在某 CTR 业务上优化一段 target attention, 最后把它从 12ms 压到了 1.4ms. 回头看, 能有这么大空间, 根子上是两条"通用 SDPA 接不住这个数据特点"的缝隙:

- 这段 attention 的 mask 其实是**可分离**的(两个小张量的外积), 但 `F.scaled_dot_product_attention` 只能收一张完整物化的 mask — 你没法把两个因子分开传给它, 于是那张又大又冗余的 mask 每次都得物化、读一遍.
- SDPA 这类算子**不会 lower 成 Triton**, 走的是固定实现. 而它上游"跨 batch 共享"的 K/V, 为了喂给这个不透明算子, 被 `expand` + 物化成了每个 batch 一份的独立拷贝.

这两条都是很具体的数据特点(甚至可以说是 corner case), 通用算子照顾不到, 恰恰是手写 kernel 的机会. 下面按我实际排查的顺序讲: 先怀疑 mask 物化、写了自定义 kernel 却发现下降不及预期, 再回头揪出 expand 物化, 最终两处一起解决. 顺便当作一篇"怎么看 attention 访存 bound"的入门.

版本信息:

- GPU: NVIDIA RTX PRO 5000 72GB (Blackwell, SM120)
- PyTorch: 2.12.0a0, Triton: 3.7.0
- 精度: fp16
- 计时: `torch.cuda.Event` + `synchronize`, warmup 后取两次 event 间隔的平均

## 背景: 一个"可分离 mask"的 target attention

先说这个 attention 长什么样. 它是搜推里常见的 target attention: 少量 target item 去 attend 一段较长的上下文序列. 生产形态(动态 batch, 取晚高峰 `batch=1700`):

```
q     : (1700, 8, 15, 64)     # target, q 序列只有 15
k / v : (1,    8, 1458, 64)   # 上下文, kv 序列 1458, 且逻辑上跨 batch 共享
mask  : (1700, 8, 15, 1458)   # bf16 加性 mask
out   : (1700, 8, 15, 64)
```

导出的 fx graph 里, 这块大概长这样(把无关的 to.dtype 精简了一下):
...
把 mask 物化干掉了, 按理该有明显收益. 实测(cudaEvent, 就是那张图的前两行):
...
最后留个尾巴: 现在 attention kernel 是 1.40ms, q 只有 15 导致 tensor core 的 M 维只用到 16, 利用率其实还很低. 下一步可以做 batch grouping — 反正 K/V 跨 batch 共享, 把多个 batch 的 q 摞起来做一次大 matmul, 把 M 撑到 64 以上. 不过那是访存 bound 解决之后的事了, 有机会再写一篇.
