# 从分散模型到整图编译：一次时序截面推理链路的初步重构

最近新接触了一套在时序截面上存在大量并发的推理框架。

时序截面的意思是, 某个固定时刻一到, 系统会瞬间出现数十个甚至更多model的计算需求。类似的时间截面在一天中还会出现很多次, 因此这套框架的负载很有节奏: 平时相对安静, 时间一到, 一大批任务会突然一起涌进来。

这里先说“并发”, 不说“并行”。这些任务会在同一时刻被提交, 不代表它们最终会在GPU上同时执行。为了方便说明, 后文统一使用“80个前置模型、6个后置模型”作为虚构示例, 它们不对应真实业务规模。

这套框架已经稳定跑了很久, 中间也叠加了不少针对业务特点的优化。不过当模型数量、数据频率和显存占用一起往上涨时, 原来一些还算堪用的设置开始漏风了。房子住久了就该翻新, 我也打算给这套玩意翻修翻修。
...
如果单模型都没有变快, 甚至还变得更占显存, 那么后面对框架做再大的手术也没有意义。我们先从单个模型开始梳理, 再回到整条时序截面链路, 看看单模型省下来的时间为什么会被分散的模型调度重新吃掉。

## 背景: 时间一到, 模型集中开跑

每个时间片到达时, 框架会集中触发一批模型。这些模型并不完全独立, 而是分成前后两层:

- 绝大多数是前置模型, 时间片一到就可以立即执行。
- 少量是后置模型, 必须等前置模型全部完成, 再使用前者的输出继续计算。

原框架使用线程池执行这套流程。先把大量前置模型全部提交进去, 通过Future等待它们完成；收齐并整理输出以后, 再把少量后置模型提交到线程池。
...
一次截面推理中的大部分模型都集中在前置阶段。为了尽快把这些任务送上GPU, 使用线程池并发提交TorchScript模型, 从CPU编程的视角看是一种很自然的设计。

### 挑战一: 并发不是免费的

线程池可以让多个任务同时向前推进, 但线程数量增加并不等于GPU利用率会等比例增加。

对于这批以小模型为主的任务, 一次前向经常不到1ms。此时端到端耗时里不只有矩阵乘法, 还有Python线程池调度、每个TorchScript Module的调用与参数封装, 以及一串小kernel的launch。尤其是数据的重新组织: 前置模型全部结束后, 需要对输出进行重排, 才能满足后置模型的输入要求。在Python runtime中通过索引重新组织数据, 本身就是一项不小的开销。

### 挑战二: 还没开始算, 显存先快装满了

由于每个时间片使用的模型都不一样, 时间片间隔又非常短, 来不及临时加载, 因此程序启动时就需要加载全部模型。

线上使用多张数据中心GPU。当前所有模型常驻以后, 模型本身就会吃掉单卡大半显存, 还没算并发推理的波峰。这还是大量模型权重已经使用低精度格式以后的结果。后续模型数量继续增加时, 留给输入、输出、中间workspace和并发执行的空间只会越来越少。

我们最终要同时回答两个问题:

- 高频、小batch和高并发下, 新后端能不能减少单模型执行开销?
...
| --- | ---: | ---: |
| 小型FP32前置模型 | 1.94x | - |
| BF16前置模型 | 1.36x | 1.49x |
| 其余FP32前置模型 | 1.02x | 1.02x |
| FP32后置聚合模型 | 0.994x | 1.01x |

结果没有整齐到“所有模型一律快30%”, 反而更接近真实世界。

小型FP32前置模型收益最明显, BF16前置模型也有比较稳定的收益。其余FP32前置模型大多只是略快, 后置聚合模型整体则接近持平。
...
| 模型类型 | 观察 |
| --- | --- |
| 小型FP32前置模型 | AOTI workspace有所增加 |
| 其余前置模型 | AOTI更低 |
| 后置聚合模型 | AOTI约为原来一半 |

小型FP32前置模型的临时workspace有所上涨, 但它的模型常驻部分下降更多, 整卡峰值最终仍然更低。
...
BF16前置模型观察到的最大绝对误差为4.0。单看这个数字确实有点吓人, 但对应样本的输出绝对值最大达到812, 平均绝对值约105.57:
...
所以结论不是“stage-1误差很大”, 而是**输出本身数值较大, 绝对误差被BF16的表示粒度放大了；换成相对误差后, 结果与BF16精度预期一致**。
...
2. 对小型FP32前置模型和BF16前置模型, AOTI已经给出了比较稳定的性能收益。
...
沿用前面的虚构规模, 80个前置模型和6个后置模型之间存在上下游依赖关系, 执行过程如下:
...
```python
first_jobs = []
for model, columns in first_models:
    model_input = all_features[:, columns]
...
第一批先哼哧哼哧塞进线程池, 主线程守着Future等结果; 收齐以后再哼哧哼哧拼Tensor、做重排; 最后把第二批重新塞回线程池。再考虑异常处理、设备分配、模型缓存和多时间截面, 真正的代码只会比这更绕。

<!--
ChatGPT App重绘prompt：
请生成一张16:9横版、用于中文GPU工程博客的手绘卡通信息图。暖白色纸张背景, 马克笔和蜡笔质感, 线条轻微抖动, 柔和的红蓝绿橙配色, 画面专业但有一点幽默感。画面从左到右表达：一个时钟敲响表示时间截面到来；一块很大的彩色表格Tensor；许多小模型从Tensor里抽取不同颜色的列, 被一群忙碌的CPU小人塞进线程池；中间有一个收集Future票据、等待全部完成并重新整理彩色方块的工作台；随后6个模型进入第二个线程池；最右边是一块GPU, GPU内部只有一条窄窄的传送带, 不同颜色的kernel方块排成一列且中间有明显空隙。重点表现“CPU线程很多, GPU仍然只有一条队伍”。不要出现任何项目名、业务名、公司名、logo或水印。图片内部不要生成文字, 只用图形、数字80、数字6和箭头表达流程, 留出干净边距。
如果采用ChatGPT生成图, 建议保存为time-slice-thread-pool-chatgpt.png, 并替换下一行图片引用。
-->

![时间截面到来后的线程池执行方式](time-slice-thread-pool.svg)

一天内会有密集的时间截面。同样一套线程池、Future、拼接和重排会反复出现, 模型越多, 复杂度越高。
...
既然这些模型本来就有明确的数据依赖, 那就不要继续靠Python线程池来表达这层依赖。我们把原来的多张FX Graph融合成一张更大的FX Graph, 80个前置模型的计算、输出整理以及6个后置模型之间的关系全部写进图里, 然后把整张图一次性交给AOTInductor编译。
...
<!--
ChatGPT App重绘prompt：
请生成一张16:9横版、用于中文GPU工程博客的手绘卡通信息图。暖白色纸张背景, 马克笔手绘风, 线条轻微抖动, 柔和的红蓝绿橙配色。画面从左到右表达：左侧散落着许多张小计算图纸, 每张图纸上只有几个圆点和箭头, 周围散落着索引小纸条；这些小图被一个漏斗或双手合并成中间一张很大的FX Graph卷轴, 大图内部清楚画出共享输入、静态分支、前后依赖和6条输出支路；大图进入一个像熔炉一样的编译器, 有齿轮和火花；右侧输出一个整齐的软件包和C++ wrapper, 最后送进GPU。GPU内部是一条连续紧凑的kernel传送带, 方块之间几乎没有空隙。底部用四个无文字图标分别暗示：减少H2D、减少小kernel、跨图融合、内存复用。不要出现项目名、业务名、公司名、logo或水印。图片内部不要生成文字, 只使用图形和箭头, 留出干净边距。
...
每个模型选择哪些输入列、第一批输出应该按照什么顺序交给第二批, 这些信息在运行前就是固定的。留在Python里时, 它们表现为list、NumPy array、H2D和一串index kernel; 进入大图以后, 编译器看到的是常量。固定列选择可以固化进codegen的地址计算, 也可以融合进后续kernel; 固定重排则可以让前一个kernel直接写到目标位置, 不必先写一份结果再单独搬一次。
